...
import { ServiceFormShell, ClaimRail, MedialaneCollectionCard, CollapsibleSection, Label } from "@medialane/ui";
...
export function DataTokenizationTask() {
  ...
  async function run() {
    ...
    try {
      ...
      setProgress("Confirm the issuance in your wallet");
      const result = await issueToRecipients(signer, {
        service: DATA_TOKENIZATION_SERVICE,
        owner: address,
        recipients: recipients.map((r) => r.value),
        tokenUri,
        collectionContract: values.collectionId,
      });
        ...
        aside={
          <>
            <MedialaneCollectionCard
              ...
              creator={address ? short(address) : ""}
            />
            <ClaimRail
              included={[
                { icon: FileCheck2, title: "Proof of ownership", desc: "Authorship and date are recorded permanently." },
                { icon: Scale, title: "Terms that travel", desc: "Licensing is carried by the asset wherever it goes." },
                { icon: Users, title: "Held by the right people", desc: "Each recipient owns their copy outright." },
              ]}
              steps={[
      ...
      >
        <div className="space-y-8">
          ...
          <section className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                One email per line. Everyone receives their own copy — the app deploys a wallet for
                anyone who doesn&apos;t have one yet.
              </p>
            </div>
          ...
          <div className="flex items-center gap-3">
            <Button onClick={run} disabled={busy || !address} size="lg">
              {busy ? (
                <>
            ...
            {!address ? (
              <span className="text-sm text-muted-foreground">Sign in to issue.</span>
            ) : null}
          </div>
        </div>
      </ServiceFormShell>
